#!/usr/bin/env bash
# Run via `pixi run build-robot-view` (from this directory) so `em++` is
# on PATH — see pixi.toml. Needs demo_env/ assembled first (see
# ../docs/demo_env.md).
set -eo pipefail
...
command -v em++ >/dev/null || { echo "em++ not found — run this via 'pixi run build-robot-view', not directly." >&2; exit 1; }

mkdir -p "$DEMO_DIR/out"

# Same geometry_msgs LIBS set as build_teleop.sh (identical message type,
# just subscribing instead of publishing).
LIBS=(
...
em++ \
  ...
  "$DEMO_DIR/robot_view.c" \
  "$DEMO_DIR/wasm_link_stubs.c" \
  "${LIBS[@]}" \
  -o "$DEMO_DIR/out/robot_view.js"

cp "${LIBS[@]}" "$DEMO_DIR/out/"
cp "$DEMO_DIR/robot_view.html" "$DEMO_DIR/demo-page.css" "$DEMO_DIR/out/"
