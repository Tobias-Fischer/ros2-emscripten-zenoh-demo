#!/usr/bin/env bash
# Run via `pixi run build-camera` (from this directory) so `em++` is on
# PATH — see pixi.toml. Needs demo_env/ assembled first (see
# ../docs/demo_env.md), including sensor_msgs (added there for the GPS
# demo, reused here).
set -eo pipefail
...
command -v em++ >/dev/null || { echo "em++ not found — run this via 'pixi run build-camera', not directly." >&2; exit 1; }
