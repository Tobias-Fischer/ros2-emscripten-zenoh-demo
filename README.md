...
generated by CI) plus a small JupyterLite deployment
(`site/jupyterlite/`, built with `jupyterlite-core` +
`jupyterlite-pyodide-kernel`) with one notebook showing a much lighter way
to talk to a zenoh-routed ROS 2 graph — plain HTTP against `zenohd`'s REST
plugin, no compiled wasm required.
...
**Not yet reproducible from CI**: `site/assets/` and `site/jupyterlite/`
were built locally and copied in — rebuilding them requires the full
`ros-rolling` emscripten-wasm32 recipe closure and a local JupyterLite
build. Turning that into a `pixi`-driven GitHub Actions job (so the whole
site, including the compiled demos, is rebuilt reproducibly rather than
carrying hand-copied binaries) is a planned follow-up, not yet done.
