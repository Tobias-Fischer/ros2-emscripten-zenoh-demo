...
| 3c | [esol-community/rmw_zenoh_pico](https://github.com/esol-community/rmw_zenoh_pico) | [#12](https://github.com/esol-community/rmw_zenoh_pico/pull/12) | Adds real QoS event/matching support: a session-wide liveliness subscriber (`zenoh_pico_graph_cache.c`) discovers other peers' publishers/subscriptions on the same topic and checks QoS compatibility, driving the previously-dead `RMW_EVENT_{REQUESTED,OFFERED}_QOS_INCOMPATIBLE`/`{SUBSCRIPTION,PUBLICATION}_MATCHED` events and implementing `rmw_*_count_matched_*()` (previously `RMW_RET_UNSUPPORTED` stubs). Also fixes two bugs this was the first thing to exercise hard enough to hit: `rmw_wait()`'s events loop cast `events->events[i]` straight to the RMW-internal event manager type, when rcl's own `rcl_wait_set_add_event()` actually stores the `rmw_event_t*` handle there (confirmed against `rcl`'s and `rmw_fastrtps`'s own source) — reinterpreting that struct's bytes as a manager made every registered event look permanently "changed", which is what caused an intermittent `rclpy` executor crash (`_WorkTracker`) this project hit the first time a real event reached it; and `rmw_take_event()` reported *any* change on a shared manager as belonging to whichever specific event type it was asked about, so a genuine `MATCHED` event could spuriously fire a sibling `OFFERED_QOS_INCOMPATIBLE` callback that never actually happened. Also implements `rmw_service_server_is_available()` (previously another `RMW_RET_UNSUPPORTED` stub — `rclpy`'s `service_is_ready()`/`wait_for_service()` raised instead of returning `False`) by reusing the same liveliness-discovery graph cache: local clients register themselves the same way publishers/subscriptions do, and the liveliness handler now also recognizes `"SS"` (service) tokens and matches them by topic name/type. |
...
| 4b | [micro-ROS/rosidl_typesupport_microxrcedds](https://github.com/micro-ROS/rosidl_typesupport_microxrcedds) | [#86](https://github.com/micro-ROS/rosidl_typesupport_microxrcedds/pull/86) | Two real bugs in the generated `cdr_deserialize()`, both platform-independent: (1) a genuine heap buffer overflow — deserializing a `string` field longer than its destination's current capacity (typically 1 byte, from a freshly-constructed message) `memcpy`s the full incoming length in regardless, since micro-CDR's own capacity check only sets an out-of-band error flag rather than stopping the copy; (2) a sibling bug one level up — an *unbounded* `sequence<string>` (e.g. `string[]`) has its own separate outer capacity (how many strings, not how many bytes) that was never grown at all, so any non-empty unbounded string sequence failed to deserialize unconditionally. Both fixed by peeking the incoming size first and growing the destination (`realloc()` for a string's bytes, `rosidl_runtime_c__String__Sequence__init()` for the sequence itself) before the real deserialize call. A third, analogous gap for *unbounded* `sequence<NamespacedType>` (nested-message arrays, e.g. `ParameterValue[]`) is fixed the same way, via a zero-filled `calloc()` sized from the pointee type's own `sizeof()` rather than `Sequence__init()` (unavailable for an arbitrary sibling message package without a new link-time dependency). A fourth, more severe sibling bug — the same class of real heap overflow as (1), for *unbounded* `sequence<BasicType>` (e.g. `float64[]`) rather than strings — is fixed with a plain `realloc()` before the real deserialize call. |
...
None of 1–5 (or 3b/3c/4b) are RoboStack-specific hacks kept only as local patches
